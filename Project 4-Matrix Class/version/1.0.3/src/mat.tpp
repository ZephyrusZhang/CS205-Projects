...
template<>
Mat<float> Mat<float>::operator+(const Mat<float> &other)
{
    if (this->rows != other.rows || this->cols != other.cols) {
        cout << "Inconsistent size of two matrices for addition. Exit" << endl;
        exit(EXIT_FAILURE);
...
template<>
Mat<int> Mat<int>::operator+(const Mat<int> &other)
{
    if (this->rows != other.rows || this->cols != other.cols) {
        cout << "Inconsistent size of two matrices for addition. Exit" << endl;
        exit(EXIT_FAILURE);
...
template<typename Tp>
void Mat<Tp>::writeTo(const char *path)
{
    FILE *fp;
...
template<typename Tp>
Mat<Tp> Mat<Tp>::strassen(Mat<Tp> &mat_A, Mat<Tp> &mat_B)
{
    size_t n = mat_A.rows;
    if (n <= 256) {
...
template<typename Tp>
Mat<Tp> Mat<Tp>::merge(Mat<Tp> &C11, Mat<Tp> &C12, Mat<Tp> &C21, Mat<Tp> &C22)
{
